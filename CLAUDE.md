...
- **Boulon — lit voxel room-by-room tower climber.** You climb a tower of stacked **rooms**: destroy every **static** robot in a room to open its **top gate**, dodging a **rising floor (crusher)** — get pinned between it and the ceiling/a pillar and you die instantly. Each room has its own pillars, enemies, and **two pickups** (easy + hard) granting **stacking run upgrades** (`src/upgrades.ts`, seed of a future skill tree). (It started as a twin-stick arena, then a forced-scroll shmup — both scrapped; don't reintroduce a second aim stick or a continuous auto-scroll.) Like Veillée it breaks the `MeshBasicMaterial` default for a lit toy-robot look (`MeshLambertMaterial` + `flatShading: true`, rig in `src/scene.ts` from `config.ts`); assets are kept **small** (`PLAYER.scale`/`ENEMY.scale`). Physics is **matter-js in the ground plane**: matter `(x, y)` → three `(x, 0, z)`. **Ascent = matter −y** (the ~56° tilt projects −y to the top of the screen); rooms stack toward −y, the camera **frames the current room** and pans on change (`src/scene.ts`). **Gravity is OFF**; don't add it or flip the mapping to XY. **Movement is velocity-seek** (`src/player.ts` seeks the finger target so solid pillars/walls/the crusher resolve in matter — don't switch it back to teleport-setPosition) + **auto-fire that auto-aims the nearest enemy** (`src/main.ts`); single finger, no joysticks. Per-room geometry + deterministic enemy/item placement is `src/rooms.ts` (mulberry32 seeded by room index); the rising floor is `src/crusher.ts`. The robot factory (`src/robotFactory.ts`) builds hero + enemies. The **debris budget** (`config.ts` `DEBRIS`) is a first-order perf constraint — kill-bursts (`src/debris.ts`, kinematic) must respect it; 60fps always wins over spectacle.
